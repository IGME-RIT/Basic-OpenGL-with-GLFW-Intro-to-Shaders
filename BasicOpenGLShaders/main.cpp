...
struct shape {
	std::vector<vec3> vertices; //Here we use a vector (similar to C# lists) instead of an arry to store our vertices as it is more flexible.
	GLuint VAO;
	GLuint VBO;
};
...
int main()
{
	...
	triangle.vertices = {
		...
		vec3(0.5, -0.5f, 0.0f),		vec3(0.0f, 1.0f, 0.0f),
	...
	glBufferData(GL_ARRAY_BUFFER, 
		sizeof(vec3)*triangle.vertices.size(), //The data size is the size of one vec3 multiplied by the number of vertices
	...
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(vec3) * 2, (GLvoid *)(sizeof(vec3)));
